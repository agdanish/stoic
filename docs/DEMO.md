...
3. Have a terminal ready for `npm test` (shows **483 passing**, incl. the look-ahead-bias suite, the
...
**Screen:** in terminal, run **`npm test`** → **483 passing**. Point to the suites
...
| **Demo / presentation** | This 2:30–3:00 script; committed dashboard screenshots; `npm test` (483) run on camera; one-command reproduction. |
...
- **CMC's Fear & Greed is wired LIVE into the evaluated product** as the contrarian regime gate (keyed
  ...
  and all **483** tests stay byte-reproducible.
...
npm test                    # 483 tests: determinism / BVA / look-ahead-bias / momentum / ablation / CMC-on-vs-off / cassette

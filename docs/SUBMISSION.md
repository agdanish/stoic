...
- **Reproducible** — `npm install` → `npm run fetch-data` → `npx ts-node backtest/momentum.ts` (headline) / `npx ts-node backtest/ablation.ts` (layer attribution) / `npm run backtest` (frozen anchor) → `npm test` (436 passing); setup in `README.md`; 2:30–3:00 video.

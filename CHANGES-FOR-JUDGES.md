...
| Test suite | `npx mocha` | **436 passing** (was 218 at the eval, 302 after the risk-overlay work, 416 before the ablation; +218 net-new incl. 21 momentum + 20 ablation tests) |
...
$ npx mocha   # → 436 passing; test/momentum.test.ts pins: selection in-sample only (NOT the OOS
...
$ npx mocha   # → 436 passing; cmcAdvisory + cmcLive cassette tests green.
```
...
$ npx mocha   # → 436 passing; crossSectional + ablation truncation-invariance + honesty tests green.
```
...
| Test suite 436 passing | README, DEMO.md, SUBMISSION.md, CHANGES, this snapshot | `npx mocha` (416 baseline + 20 ablation) | ✅ exact |
...
7. **~~`ROBUSTNESS-momentum.md:39` cited "416 passing"~~ — RESOLVED.** Updated to **436 passing** (both occurrences, lines 39 and 171), verified by `npx mocha`. The substantive robustness numbers in that doc are unchanged and correct. (Gap 4)
